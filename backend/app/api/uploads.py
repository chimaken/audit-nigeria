...
async def create_upload(
    ...
    try:
        blur = image_service.assess_blur_with_zoom(image_bytes)
    ...
    try:
        phash_str = image_service.calculate_phash(image_bytes)
    except OSError as e:
    ...
    if metadata is not None and metadata.strip():
        ...
        if not isinstance(parsed, dict):
            raise HTTPException(
                status_code=400, detail="metadata JSON must be an object"
            )
        metadata_obj = parsed
    ...
    if pu_id is None:
        ...
        mime = _mime_from_filename(file.filename)
        try:
            extraction = await ai_service.extract_results_from_image_bytes(
                image_bytes, mime=mime
            )
        except Exception as e:  # noqa: BLE001
            raise HTTPException(
                status_code=503,
                detail=f"Vision extraction failed (required for AI location): {e}",
            ) from e
    ...
    type_part = _slugify_fs(election.type.value)
    state_part = _slugify_fs(pu_for_path.lga.state.name)
    lga_part = _slugify_fs(pu_for_path.lga.name)
